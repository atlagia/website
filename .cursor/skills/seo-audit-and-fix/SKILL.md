...
description: Orchestrates a full SEO audit and fix by delegating each phase to specialist subagents. Use when the user says "SEO audit", "fix SEO", "optimize for search", "website SEO", or wants "perfect website SEO". Main agent runs Phase 0 only; seo-audit, seo-fix-high, seo-fix-medium-low, and seo-verify subagents run in sequence. Final result is a verified, SEO-optimized website.
---
...
3. Create a **todo list**: Phase 0 (done), seo-audit, seo-fix-high, seo-fix-medium-low, seo-verify.
...
Run these **one after another** (foreground). Invoke with the Task tool (`mcp_task`). If your environment supports custom subagent types, set `subagent_type` to `seo-audit`, `seo-fix-high`, `seo-fix-medium-low`, or `seo-verify` (see .cursor/agents/seo-audit.md, seo-fix-high.md, seo-fix-medium-low.md, seo-verify.md). Otherwise use `subagent_type: generalPurpose` and the prompt templates in the next section (include context and "You are the seo-audit specialist…" etc. in the prompt).
...
3. Audit in this order: Technical (crawlability, indexing, Core Web Vitals, mobile, HTTPS) → On-page (title, meta, H1, headings, keyword placement) → Content and E-E-A-T → Featured snippet readiness → Internal linking → Images → Structured data → Data/code (index_en.json, BaseHead, components that output meta/headings).
...
Instructions:
1. Read .cursor/skills/seo-audit-and-fix/SKILL.md (Phase 1–4 Reference section) and reference.md.
2. Fix ALL high-priority issues from the audit report, in this order: (1) Crawlability/indexing (robots.txt, canonicals, noindex, key pages linked), (2) Core Web Vitals (LCP, INP, CLS), (3) Critical on-page (title and meta on homepage/main landing, single H1, keyword in first 100 words and one H2), (4) Security/HTTPS if applicable.
...
Instructions:
1. Read .cursor/skills/seo-audit-and-fix/SKILL.md (Phase 1–4 Reference section) and reference.md.
2. Re-run the audit checklist on the fixed areas (or run Lighthouse/technical checks where applicable). If UI or visible content changed, use MCP browser (cursor-ide-browser): navigate to baseUrl and key pages (e.g. /en), take screenshots, confirm titles/headings/visibility.
...
1. **You never do Phases 1–4 yourself.** You only run Phase 0 and delegate seo-audit → seo-fix-high → seo-fix-medium-low → seo-verify in that order.
...
3. **Update context between steps.** If a subagent returns e.g. "themePath is …" or "port 7010", add that to the context you pass to the next subagent. Always pass the audit report to fix-high and fix-medium-low; pass both fix summaries to seo-verify.
4. **After seo-verify succeeds**, tell the user the site has achieved perfect website SEO and give a short summary of fixes. If seo-verify reports remaining issues, either run another fix cycle or report the list to the user.
...
- **Phase 2 (seo-fix-high):** Fix crawlability/indexing, Core Web Vitals, critical on-page (title, meta, H1, keyword), security. No medium/low yet.
...
### 1. Technical (Google Search Essentials)
...
- **Indexing:** Canonical tags; no accidental `noindex` on key pages; sitemap present and submitted (if applicable).
...
- [ ] Launch **seo-verify** (generalPurpose) with context + audit report + both fix summaries; wait; capture verification result.
- [ ] If verified: tell user "perfect website SEO" and summarize fixes. If not: report remaining issues or retry.
...
Full checklists, scoring, and report template: [reference.md](reference.md).
