...
const ETHER_COLORS = ['#004D33', '#00E599', '#065F46'];
      ...
      <div className="relative z-10 text-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <AnimatedContent direction="down" delay={100} duration={600}>
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/[0.07] backdrop-blur-md border border-white/10 mb-8">
            ...
            <DecryptedText
              text="AI-Powered SEO Automation Platform"
              speed={30}
        ...
        <AnimatedContent direction="up" delay={600} duration={800}>
          <div className="flex flex-col sm:flex-row justify-center gap-4 mb-16">
            <a
              ...
              className="group inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-black font-semibold shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 transition-all duration-300 hover:-translate-y-0.5"
            >
              {cta.primary.text}
          ...
          {Object.entries(stats).map(([key, value], i) => {
            ...
            return (
              <AnimatedContent key={key} direction="up" delay={800 + i * 150} duration={600}>
                <div className="group relative p-5 sm:p-6 rounded-2xl bg-white/[0.06] backdrop-blur-md border border-white/10 hover:border-emerald-500/30 transition-all duration-300 hover:shadow-[0_0_30px_rgba(0,229,153,0.15)]">
                  <p className="text-xl sm:text-2xl md:text-3xl font-bold text-white">
                    {isNumeric ? <><CountUp to={num} duration={2500} suffix={suffix} /> </> : null}
                    {isNumeric ? label : value}
                  </p>
                  <p className="text-sm text-gray-300 capitalize mt-1">{key}</p>
                </div>
              </AnimatedContent>
            );
          })}
