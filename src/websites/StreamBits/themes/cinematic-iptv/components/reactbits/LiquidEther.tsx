  ...
  useEffect(() => {
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        u.force.value.set(forceX, forceY); u.center.value.set(cX, cY); u.scale.value.set(cs, cs);
        super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: any[]) {
        const { viscous: vi, iterations, dt: d } = (args[0] || {}) as any;
        if (!this.uniforms) return;
        let fbo_out: any;
        if (typeof vi === 'number') this.uniforms.v.value = vi;
    ...
    class Poisson extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        let p_out: any; const iter = iterations ?? 0;
        for (let i = 0; i < iter; i++) {
    ...
    class Pressure extends ShaderPass {
      ...
      update(...args: any[]) {
        const { vel, pressure: pr } = (args[0] || {}) as any;
        if (this.uniforms && vel && pr) { this.uniforms.velocity.value = vel.texture; this.uniforms.pressure.value = pr.texture; }
        super.update();
      }
    }
