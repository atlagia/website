...
class CarLights {
  ...
  constructor(webgl: AppInstance, options: HyperspeedOptions, colors: number[] | THREE.Color, speed: [number, number], fade: THREE.Vector2) {
    this.webgl = webgl;
...
class LightsSticks {
  ...
  constructor(webgl: AppInstance, options: HyperspeedOptions) { this.webgl = webgl; this.options = options; }
  ...
  init() {
    ...
    let colorArray: THREE.Color[];
    if (Array.isArray(options.colors.sticks)) { colorArray = (options.colors.sticks as any).map((c: number) => new THREE.Color(c)); } else { colorArray = [new THREE.Color(options.colors.sticks)]; }
...
class Road {
  ...
  constructor(webgl: AppInstance, options: HyperspeedOptions) { this.webgl = webgl; this.options = options; this.uTime = { value: 0 }; }
...
class AppInstance {
  ...
  dispose() {
    this.disposed = true;
    if (this.scene) { this.scene.traverse(object => { const obj = object as any; if (!obj.isMesh) return; if (obj.geometry) obj.geometry.dispose(); if (obj.material) { if (Array.isArray(obj.material)) obj.material.forEach((m: any) => m.dispose()); else obj.material.dispose(); } }); this.scene.clear(); }
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    const myApp = new AppInstance(container, options);
    appRef.current = myApp;
    myApp.loadAssets().then(myApp.init);
    return () => { if (appRef.current) appRef.current.dispose(); };
  }, [effectOptions]);

  return <div ref={hyperspeed} style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'absolute' }} />;
};
