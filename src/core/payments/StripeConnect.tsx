...
const StripeConnectPaymentForm: React.FC<StripeConnectPaymentProps> = ({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      // Use seller account ID from props or default to env config
      const accountId = sellerAccountId || 
        process.env.STRIPE_CONNECT_ACCOUNT_ID;

      // Calculate application fee
      const applicationFee = applicationFeePercent || 
        parseFloat(process.env.STRIPE_PLATFORM_APPLICATION_FEE_PERCENT || '0.5');

      // Create payment intent on the server with Connect account
      const response = await fetch('/api/payments/create-connect-intent', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          amount: total,
          currency: currency || process.env.STRIPE_PLATFORM_CURRENCY || 'usd',
          sellerAccountId: accountId,
          applicationFeeAmount: total * (applicationFee / 100)
        })
      });
